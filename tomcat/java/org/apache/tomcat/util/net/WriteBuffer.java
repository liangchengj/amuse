...
public class WriteBuffer {
    ...
    boolean write(SocketWrapperBase<?> socketWrapper, boolean blocking)
            throws IOException {
    ...
    public boolean write(Sink sink, boolean blocking)
            throws IOException {
        Iterator<ByteBufferHolder> bufIter = buffers.iterator();
        boolean dataLeft = false;
        while (!dataLeft && bufIter.hasNext()) {
    ...
    public interface Sink {
        boolean writeFromBuffer(ByteBuffer buffer, boolean block)
                throws IOException;
    }
}
