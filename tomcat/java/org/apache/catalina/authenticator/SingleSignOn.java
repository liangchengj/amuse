...
public class SingleSignOn extends ValveBase {
    ...
    protected void deregister(String ssoId) {
        ...
        if (sso == null) {
            if (containerLog.isDebugEnabled()) {
                containerLog.debug(sm.getString(
                        "singleSignOn.debug.deregisterFail",
                        ssoId
                ));
            }
            return;
        ...
        if (ssoKeys.size() == 0) {
            if (containerLog.isDebugEnabled()) {
                containerLog.debug(sm.getString(
                        "singleSignOn.debug.deregisterNone",
                        ssoId
                ));
            }
        }
    ...
    private void expire(SingleSignOnSessionKey key) {
        if (engine == null) {
            containerLog.warn(sm.getString(
                    "singleSignOn.sessionExpire.engineNull",
                    key
            ));
            return;
        }
        ...
        if (host == null) {
            containerLog.warn(sm.getString(
                    "singleSignOn.sessionExpire.hostNotFound",
                    key
            ));
            return;
        }
        ...
        if (context == null) {
            containerLog.warn(sm.getString(
                    "singleSignOn.sessionExpire.contextNotFound",
                    key
            ));
            return;
        }
        ...
        if (manager == null) {
            containerLog.warn(sm.getString(
                    "singleSignOn.sessionExpire.managerNotFound",
                    key
            ));
            return;
        }
        ...
        try {
            ...
            containerLog.warn(sm.getString(
                    "singleSignOn.sessionExpire.managerError",
                    key
            ), e);
            return;
        }
        if (session == null) {
            containerLog.warn(sm.getString(
                    "singleSignOn.sessionExpire.sessionNotFound",
                    key
            ));
            return;
        }
    ...
    protected void removeSession(String ssoId, Session session) {
        ...
        if (containerLog.isDebugEnabled()) {
            containerLog.debug(sm.getString(
                    "singleSignOn.debug.removeSession",
                    session,
                    ssoId
            ));
        }
