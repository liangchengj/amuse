...
public class Tomcat {
    ...
    public static void initWebappDefaults(Context ctx) {
        ...
        // Welcome files
        ctx.addWelcomeFile("main");
        ctx.addWelcomeFile("main.html");
    ...
    private static boolean changeToGbkLog() {
        if (OS.IS_WIN32 || OS.IS_WIN64
                /*System.getProperty("os.name").toLowerCase().startsWith("win")*/) {
            return !"utf-8".equalsIgnoreCase(System.getProperty("file.encoding"));
        }
        return false;
    }
    ...
    static {
        ...
        if (JreCompat.isGraalAvailable()) {
            if (changeToGbkLog()) {
                Tomcat.LOGGING_PROPERTIES_TEXT.replace(
        ...
        InputStream is = null;
        try /*(*//*InputStream is = new ByteArrayInputStream(Tomcat.LOGGING_PROPERTIES_TEXT.getBytes())*//*
         *//*new FileInputStream(new File(
                                 System.getProperty("java.util.logging.config.file", "conf/logging.properties")
                         ))*//*)*/ {

            if (changeToGbkLog()) {
                Tomcat.LOGGING_PROPERTIES_TEXT = new String(
                        Tomcat.LOGGING_PROPERTIES_TEXT.getBytes("utf-8"),
                        "gbk"
                );
            }
            is = new ByteArrayInputStream(Tomcat.LOGGING_PROPERTIES_TEXT.getBytes());
    ...
    private static String LOGGING_PROPERTIES_TEXT = "# Licensed to the Apache Software Foundation (ASF) under one or more\n" +
            "# contributor license agreements.  See the NOTICE file distributed with\n" +
            "# this work for additional information regarding copyright ownership.\n" +
            "# The ASF licenses this file to You under the Apache License, Version 2.0\n" +
            "# (the \"License\"); you may not use this file except in compliance with\n" +
