...
//    }
        ...
        if (ref == null) {
            ...
            while (portcomponent.hasNext()) {
                ...
                StringRefAddr refAddr =
                        new StringRefAddr(ServiceRef.SERVICEENDPOINTINTERFACE, serviceendpoint);
                ref.add(refAddr);
            ...
            while (handlers.hasNext()) {
                ...
                for (int i = 0; i < handler.getSoapRolesSize(); i++) {
                    handlerRef.add(new StringRefAddr(
                            HandlerRef.HANDLER_SOAPROLE,
                            handler.getSoapRole(i)
                    ));
                }
                for (int i = 0; i < handler.getPortNamesSize(); i++) {
                    handlerRef.add(new StringRefAddr(
                            HandlerRef.HANDLER_PORTNAME,
        ...
        if (("javax.sql.DataSource".equals(ref.getClassName()) ||
            ...
            try {
                ...
                Registry.getRegistry(null, null)
                        .registerComponent(actualResource, on, null);
    ...
    public void addResourceLink(ContextResourceLink resourceLink) {
        ...
        Reference ref = new ResourceLinkRef(
                resourceLink.getType(),
                resourceLink.getGlobal(),
                resourceLink.getFactory(),
                null
        );
